...
import { ChevronDown, ChevronUp, Cookie, Settings, Shield, Bell, Trash2, RefreshCw } from "lucide-react"
import Navbar from "@/components/navbar"
import Footer from "@/components/footer"
import Link from "next/link"

export default function CookiePolicyPage() {
  const [openSection, setOpenSection] = useState<string | null>("what-are-cookies")
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 md:py-28">
        <div className="text-center">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            Last Updated: December 1, 2024. Learn how ChoiceX uses cookies and similar technologies.
          </p>
        </div>
      </section>
        ...
        <div className="prose prose-lg dark:prose-invert max-w-none">
          <div className="bg-card border border-border rounded-xl p-8 mb-8">
            <p className="text-muted-foreground mb-6">
              This Cookie Policy explains how ChoiceX uses cookies and similar technologies to recognize you when you visit our platform. It explains what these technologies are and why we use them.
            </p>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              <div className="space-y-6">
                {Object.entries(cookiePreferences).map(([type, enabled]) => (
                  <div key={type} className="flex items-center justify-between p-4 bg-background rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        {getCookieIcon(types)}
                        <h4 className="font-bold text-foreground capitalize">{type} Cookies</h4>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        {getCookieDescription(types)}
                      </p>
                    </div>
                    <div className="flex items-center gap-4">
                      {type === "necessary" ? (
          ...
          <div className="mt-12">
            <h3 className="text-2xl font-bold text-foreground mb-6">Detailed Cookie Information</h3>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-card">
                    <tr>
                      <th className="text-left p-4 font-bold text-foreground">Cookie Name</th>
                      ...
                      <th className="text-left p-4 font-bold text-foreground">Type</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {cookieDetails.map((cookie, idx) => (
                      <tr key={idx} className="hover:bg-card/50 transition">
                        ...
                        <td className="p-4 text-muted-foreground">{cookie.purpose}</td>
                        <td className="p-4 text-muted-foreground">{cookie.duration}</td>
                        <td className="p-4">
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            cookie.type === 'necessary' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                            cookie.type === 'analytics' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' :
                            cookie.type === 'functional' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300' :
          ...
          <div className="mt-12 bg-card border border-border rounded-xl p-8">
            <h3 className="text-2xl font-bold text-foreground mb-4">Browser Cookie Controls</h3>
            <p className="text-muted-foreground mb-6">
              You can control cookies through your browser settings. Here's how to manage cookies in popular browsers:
            </p>
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-bold text-foreground mb-3">Google Chrome</h4>
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li>1. Click the three dots menu → Settings</li>
                  <li>2. Select "Privacy and security" → "Cookies and other site data"</li>
                  <li>3. Choose your preferred cookie settings</li>
                </ul>
              </div>
              <div>
                <h4 className="font-bold text-foreground mb-3">Mozilla Firefox</h4>
                <ul className="space-y-2 text-sm text-muted-foreground">
          ...
          <div className="mt-8 p-6 bg-primary/5 border border-primary/20 rounded-lg">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h4 className="font-bold text-foreground mb-2">More Questions?</h4>
                <p className="text-muted-foreground text-sm">
                  Contact us at privacy@choicex.edu.et for cookie-related inquiries
                </p>
              </div>
...
const getCookieIcon = (type) => {
  switch(type) {
    case 'necessary': return <Shield className="w-5 h-5 text-primary" />
    case 'analytics': return <RefreshCw className="w-5 h-5 text-primary" />
    case 'functional': return <Bell className="w-5 h-5 text-primary" />
    case 'marketing': return <Settings className="w-5 h-5 text-primary" />
    default: return <Cookie className="w-5 h-5 text-primary" />
  }
}

const getCookieDescription = (type) => {
  switch(type) {
    case 'necessary': return 'Required for platform functionality. Cannot be disabled.'
    case 'analytics': return 'Helps us understand how users interact with the platform.'
    case 'functional': return 'Remembers your preferences and settings.'
    case 'marketing': return 'Used for personalized content and advertising.'
    default: return ''
  }
}
